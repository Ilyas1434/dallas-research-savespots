#!/usr/bin/env python3
"""
Manuscript figures: 300 dpi PNG, vector PDF, and an APA caption file each.

  Figure 1  Measured naloxone access desert -- tract choropleth of the share of
            population beyond one mile of supply, in fixed classes, over the 38
            supply points and their one-mile catchments.
  Figure 2  Composite vulnerability index -- composite score choropleth with the
            Tier 1 boundary outlined and Jenks cuts marked on the colour bar.
  Figure 3  Ranked placement strategy -- top 50 candidates sized by reach over
            muted tier shading, with the greedy coverage table inset.
  Figure 4  Overdose mortality at CDC native reporting units -- 150-unit Jenks
            choropleth with the suppressed units hatched.

Every layer is an aggregate (tract or CDC-unit choropleth, population count) or a
facility/storefront point. No individual death records exist in this repository
and none are synthesised. Geometry is handled in EPSG:32138 throughout.

Run: ./venv/bin/python scripts/export_figures.py
"""
...
MI = 1609.344
CLUSTER_LINK_M = 1500   # single-linkage radius for the Figure 3 cluster callouts
...
mpl.rcParams.update({
    ...
    "pdf.fonttype": 42,       # embed TrueType so vector text stays editable
    "ps.fonttype": 42,
    "svg.fonttype": "none",
    "hatch.linewidth": 0.4,
})
...
# Okabe-Ito, colourblind-safe; keyed to naloxone_locations.category.
SUPPLY_STYLE = {
...
def raise_legend(*legs):
    """Legends default to zorder 5, below the county line (6) and points (8+),
    which would otherwise print over the legend boxes."""
    for leg in legs:
        leg.set_zorder(30)
...
nal = nal[~nal.geometry.isna()].to_crs(CRS_M)   # drops the mobile unit (null geometry)
...
T3_UP = CUT["tier3_upper_bound"]
T1_LO = CUT["tier1_lower_bound"]
...
def figure1():
    ...
    # Fixed classes, not data-driven, so the map stays comparable across refreshes.
    ...
    box = (f"84% of residents live > 1 mile from any naloxone access point\n"
           ...
           "Conservative: the 38 points credit 13 OTP/methadone clinics and 7 nightlife\n"
           "venues whose walk-in public access is limited; the true desert is larger.")
    ...
    leg1 = ax.legend(handles=class_handles, title="% population > 1 mi\nfrom naloxone",
                     loc="lower left", bbox_to_anchor=(0.012, 0.10), fontsize=7.8,
                     title_fontsize=8.6, frameon=True, framealpha=0.95, borderpad=0.9)
    leg1._legend_box.align = "left"
    ax.add_artist(leg1)
    leg2 = ax.legend(handles=supply_handles, title="Existing naloxone supply\n(n = 38)",
                     loc="lower right", bbox_to_anchor=(0.982, 0.04), fontsize=7.7,
                     title_fontsize=8.6, frameon=True, framealpha=0.95, borderpad=0.9)
    leg2._legend_box.align = "left"
    raise_legend(leg1, leg2)
    ...
    _caption("figure1",
        ...
        ("Each of the 645 census tracts is shaded by the share of its 2020 census-block "
         ...
         "stranded (beyond one mile of supply and beyond a half-mile of transit). The 84.0% "
         "figure is conservative: the 38-point inventory credits sites with limited "
         "functional access - 13 OTP/methadone clinics that dispense chiefly to enrolled "
         "patients rather than walk-ins, and 7 nightlife venues whose dispensers are "
         "reachable only during venue hours, typically before peak overdose risk. Excluding "
         "them would enlarge the measured desert."),
...
def figure2():
    ...
    # Almost every ranked tract is count-suppressed, so hatching them would cover
    # the map. Mark the exception instead -- the unsuppressed tracts -- splitting by
    # fill brightness so the dots stay legible at both ends of the viridis ramp.
    unsup = ranked[~ranked["count_suppressed"]].copy()
    n_unsup = len(unsup)
    unsup["_nb"] = (unsup["composite_score"] - vmin) / (vmax - vmin)
    light = unsup[unsup["_nb"] < 0.55]
    dark = unsup[unsup["_nb"] >= 0.55]
    if len(light):
        light.plot(ax=ax, facecolor="none", edgecolor="#f5f5f5", linewidth=0.0,
                   hatch="....", zorder=2, alpha=0.75)
    if len(dark):
        dark.plot(ax=ax, facecolor="none", edgecolor="#2a2a2a", linewidth=0.0,
                  hatch="....", zorder=2, alpha=0.65)
    ...
    # Tiers are ranges on the continuous colour bar rather than categorical fills,
    # so they get no swatches: Tier 1 keeps its outline, Tiers 2 and 3 are keyed to
    # the cut lines drawn on the bar.
    handles = [
        Line2D([0], [0], color="#111", lw=2.6,
               label=f"Tier 1 - highest, >= {T1_LO:.3f}  ({n1} tracts, heavy outline)"),
        Line2D([0], [0], color="#111", lw=1.4,
               label=f"Tier cuts marked on colour bar:\n"
                     f"Tier 2 = {T3_UP:.3f}-{T1_LO:.3f}  ({n2} tracts)\n"
                     f"Tier 3 = < {T3_UP:.3f}  ({n3} tracts)"),
        Patch(facecolor=plt.cm.viridis(0.75), edgecolor="#2a2a2a", hatch="....",
              label=f"Death count NOT suppressed ({n_unsup} of 643\n"
                    "ranked tracts; all others CDC-suppressed)"),
    ...
    _caption("figure2",
        ...
        ("Fill colour is a composite vulnerability score, the equal-weighted mean (0.20 each) "
         ...
         "outlined in heavy black. CDC suppression is near-universal: 592 of all 645 tracts "
         "(91.8%) carry suppressed death counts, 591 of them among the 643 ranked tracts "
         "(the remaining suppressed tract is a zero-population exclusion). Fine dotted "
         f"hatching therefore marks the exception - the {n_unsup} ranked tracts whose counts "
         "are NOT suppressed; every unhatched ranked tract relies on a model-based rate. "
         "Sensitivity analysis re-running the full pipeline with re-weighted layers shows "
         "the tiering is robust to the mortality weight but sensitive to the distance "
         f"weight: doubling mortality weight leaves {md['pct_identical_tier']:.0f}% of "
         "tracts in the same tier, while doubling distance weight flips "
         f"{100 - dd['pct_identical_tier']:.1f}% of tracts - all between adjacent tiers, "
         "with zero Tier 1<->Tier 3 flips under either variant."),
...
def _single_linkage(pts, radius):
    """Group points into single-linkage clusters, returning lists of indices."""
    ...
    for a, b in cKDTree(pts).query_pairs(r=radius):
        ra, rb = find(a), find(b)
        if ra != rb:
            parent[ra] = rb
...
def figure3():
    ...
    ACC = "#0072B2"
    # Solid: agrees across both NEED variants. Hollow: composite variant only.
    ...
    # The top 50 stack into a few dense clusters, so anyone counting dots on the map
    # undercounts badly. Annotate each single-linkage cluster of 3+ with its size.
    pts = np.column_stack([top50.geometry.x.to_numpy(), top50.geometry.y.to_numpy()])
    n_annotated = 0
    for members in _single_linkage(pts, CLUSTER_LINK_M):
        if len(members) < 3:
            continue
        cx, cy = pts[members, 0].mean(), pts[members, 1].mean()
        # County bounds, not axis limits: the extent is not set yet here. Clusters
        # near the top edge flip below-left to clear the greedy-coverage box.
        bounds = county.total_bounds
        dx, dy = (3200, 3600)
        if cy > bounds[1] + 0.72 * (bounds[3] - bounds[1]):
            dx, dy = (-8500, -3000)
        ax.annotate(f"x{len(members)} sites here", xy=(cx, cy), xytext=(cx + dx, cy + dy),
    ...
    print(f"  fig3 cluster callouts cover {n_annotated} of {len(pts)} top-50 sites")

    set_extent(ax, county)
    xlim, ylim = ax.get_xlim(), ax.get_ylim()

    # Numbered by composite rank. Licence-derived category labels are omitted from
    # the map because they misread as storefront type; the caption states this.
    top5 = pr.sort_values("rank_composite").head(5).copy()
    short = {
        "SUPER FUELS LOMBARDY LLC": "1. Super Fuels Lombardy",
        "LA MICHOACANA MEAT MARKET DALLAS #30": "2. La Michoacana Meat Mkt",
        "LIRAS BARBER SHOP": "3. Liras Barber Shop",
        "FIESTA MART #77": "4. Fiesta Mart #77",
        "Fiesta Mart Fuel Station #177": "5. Fiesta Mart Fuel #177",
    }
    ...
    ax.text(0.0, 1.006,
            "Top 50 of 5,428 candidate storefronts by NEED x REACH (composite rank), sized "
            "by unserved residents reachable; vulnerability tiers shaded",
            transform=ax.transAxes, fontsize=10, color="#333", va="bottom")

    # Separate rank-agnostic analysis; its pick order differs from the map labels.
    g = pr_meta["greedy_coverage_gain"]["cumulative_summary"]
    tbl = (f"Greedy coverage of unserved residents\n"
           f"(separate rank-agnostic analysis - order differs\n"
           f"from map labels; de-overlapped; "
           f"{g['total_unserved_residents']:,} unserved total)\n"
           f"----------------------------------------\n"
    ...
    # Reference sizes sit inside the top-50 reach range so the legend markers match
    # the map. markersize is in points, hence sqrt of the scatter area.
    size_ref = [9000, 13000, 17000]
    size_handles = [
        Line2D([0], [0], marker="o", linestyle="None", color=ACC, markerfacecolor=ACC,
    ...
    type_handles = [
        ...
        Line2D([0], [0], marker="o", linestyle="None", color=ACC, markerfacecolor="none",
               markeredgecolor=ACC, markeredgewidth=1.4, markersize=10,
               label="Top-50 candidate - composite ranking only"),
        Line2D([0], [0], marker="s", linestyle="None", color="#111", markerfacecolor="none",
               markeredgecolor="#111", markeredgewidth=1.1, markersize=9,
               label="Existing naloxone supply (38)"),
    ]
    ctx_handles = [
    ...
    _caption("figure3",
        ...
        ("Candidate storefronts are ranked by NEED x REACH. REACH is the number of currently "
         ...
         f"{ag['top50_overlap_count']} of the top 50 shared). The map shows the top 50 of "
         "5,428 ranked candidates by composite score, sized by REACH; solid symbols agree "
         "in both variants, hollow symbols rank in the composite variant only. Hollow "
         "squares mark the 38 existing supply sites; muted shading shows the vulnerability "
         "tiers. Two distinct rankings appear on this figure and their orders differ: the "
         "numbered labels give the five highest candidates under the COMPOSITE ranking "
         "(1 = Super Fuels Lombardy), while the inset table summarises a separate "
         "rank-agnostic GREEDY coverage analysis (whose first pick is Burmese Asian Grocery "
         "Store) - iteratively choosing the site covering the most not-yet-covered unserved "
         "residents: the best 10, 25, and 50 non-overlapping sites would newly reach "
         "117,922, 231,806, and 388,927 of the 2,196,211 unserved residents, respectively. "
         "Candidate categories used in the ranking pipeline are licence-derived - TABC "
         "off-premise permit classes and county land-use codes - not verified storefront "
         "types (a restaurant holding an off-premise beer/wine permit is classed 'liquor'); "
         "they inform candidate discovery only and do not affect NEED x REACH scores. The "
         "top 50 concentrate in a small number of geographic clusters (count callouts on "
         "map) because REACH concentrates where large unserved populations adjoin high-need "
         "tracts."),
...
def figure4():
    ...
    # Several units sit at exactly 0, which Jenks isolates into a zero-width first
    # break. Asking for 6 classes and de-duplicating identical edges leaves 5 well
    # populated classes with usable resolution through the dense mid-range.
    breaks = sorted(set(np.round(jenkspy.jenks_breaks(vals, n_classes=6), 1)))
    n_cls = len(breaks) - 1
    colors = [plt.cm.YlOrRd(x) for x in np.linspace(0.12, 0.95, n_cls)]
    # Bounds are continuous: each class shares its upper bound with the next class's
    # lower bound, and the labels show which side is inclusive.
    class_labels = []
