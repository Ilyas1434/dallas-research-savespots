...
_Compiled 2026-07-24. Repository: `dallas research savespos`. This is the consolidated deliverable._
...
| Script | Builds |
|---|---|
| `pull_mortality.py` | `vsrr.json`, `deaths_dallas.csv`, `tract_overdose.geojson`, `deaths_by_zip.csv` (header-only), `mortality_meta.json` |
| `build_context.py` (keyed) | `svi_tracts.geojson`, `uninsured_zcta.csv`, `context_meta.json` |
...
- **Daily** (GitHub Action `daily-refresh`, `17 8 * * *` UTC, keyless): `pull_mortality → build_naloxone → build_coverage_gap → build_walk_coverage → build_composite` (via `refresh.sh`). CDC data and the Narcan feed can update any day.
- **Monthly** (`monthly-full-refresh`, `43 9 1 * *`): full `run_all.sh` chain minus `build_context` (rebuilds blight index + candidate universe + revalidates baselines). `build_context` auto-skips without `CENSUS_API_KEY`.
- **Manual (annual/biennial):** `build_context.py` (keyed) whenever ACS/SVI/DART/PIT vintages update. Both CI jobs carry **no secrets**; keyed steps run only on a machine where the key is present.
...
| `vsrr.json` | raw CDC VSRR Socrata rows (72) |
...
_End of report. Every figure is traceable to the named source + vintage; every value is tagged MEASURED, MODELED, or PUBLISHED. Any claim not reproducible from open pipeline data is marked NOT COMPUTABLE and attributed to its external source (OD2A-2024) rather than estimated._
